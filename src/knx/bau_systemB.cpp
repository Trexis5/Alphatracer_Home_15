...
size_t offset = 0; // Der Offset, den Sie ändern möchten
uint8_t value = 0; // Der Wert, den Sie setzen möchten
...
void BauSystemB::editMemory(size_t offset, uint8_t value)
{
    _memory.editMemory(offset, value);
}
void BauSystemB::editMemoryarray(size_t offset, uint8_t* values, size_t length)
{
    _memory.editMemoryarray(offset, values, length);
}
