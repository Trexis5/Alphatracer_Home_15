...
uint8_t* TableObject::save(uint8_t* buffer)
{
    ...
    if (_data) {
        ...
        //Serial.print("Data (relative): ");
        //Serial.println(_memory.toRelative(_data));
    } else {
...
const uint8_t* TableObject::restore(const uint8_t* buffer)
{
    ...
    //Serial.print("Restored Size: ");
    //Serial.println(_size);

    uint32_t relativeAddress = 0;
    buffer = popInt(relativeAddress, buffer);
    //Serial.print("Restored Relative Address: ");
    //Serial.println(relativeAddress);

    if (relativeAddress != 0) {
        _data = _memory.toAbsolute(relativeAddress);
        //Serial.print("Restored Data (absolute): ");
        //Serial.println((uintptr_t)_data); // Verwende uintptr_t f�r Zeiger
    } else {
        _data = 0;
        //Serial.println("Restored Data: 0");
    }
