...
uint8_t* Platform::getNonVolatileMemoryStart()
{
    ...
    else if(_memoryType == Callback_knx)
...
uint32_t Platform::writeNonVolatileMemory(uint32_t relativeAddress, uint8_t* buffer, size_t size)
{
    if(_memoryType == Flash)
    {
        while (size > 0)
        {
            ...
            Serial.print("Writing to Flash - Offset: ");
            Serial.print(offset);
            Serial.print(", Length: ");
            Serial.println(length);

            // Ausgabe des Buffer-Inhalts
    ...
    {
        Serial.println("Using Callback_knx for Flash Write");
...
void Platform::registerFlashCallbacks(
    ...
    {
        println("Set Callback_knx");
        _memoryType = Callback_knx;
