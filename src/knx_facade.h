...
template <class P, class B> class KnxFacade : private SaveRestore
{
  public:
    ...
    void editMemory(size_t offset, uint8_t value)
    {
        _bau.editMemory(offset, value);
    }

    void editMemoryarray(size_t offset, uint8_t* values, size_t length)
    {
        _bau.editMemoryarray(offset, values, length);
    }
