...
class ThingsBoardSized {
  ...
  private:
    ...
    inline void Firmware_Shared_Attribute_Received(const Shared_Attribute_Data& data) {
      ...
      //Serial.println("------Firmware_Shared_Attribute_Received-------------- 1");
...
//Serial.println("++++++++++ Firmware_Shared_Attribute_Received-------------- a");
...
Serial.print(F("++++++++++ Firmware_Shared_Attribute_Received-------------- b: "));Serial.println(verzoegerung_fw_update);
      ...
      if(verzoegerung_fw_update == 0) {
         verzoegerung_fw_update = 1;
               ...
               Preferences preferences;
               preferences.begin("eenvs", false); // Ordner eenvs anlegen/verwenden
               preferences.putUInt("fw_verz_update", verzoegerung_fw_update);    //  0= aus 
          
            // Zufällige Verzögerung hinzufügen (z.B. zwischen 10 Sekunden und 200 Sekunden)
            TB_newFwVersion = TB_currFwVersion;
            //randomDelay = random(10000, 200000);  //zwischen  10 Sekunden   und   3 Minuten 20 Sekunden 
            randomDelay = random(10000, 60000);  
            delayStartTime = millis();
            startFirmwareUpdate = true;
            dataFirmwareUpdate = true;

            // Debug-Ausgabe der Verzögerung in Sekunden
            Serial.print(F("Verzögerung bis zum Start des Updates: "));
            Serial.print(randomDelay / 1000);
            Serial.println(F(" Sekunden"));
            constexpr char FW_STATE_UPDATEDt[] PROGMEM = "wait";
            constexpr char NEW_FWt[] = "A new Firmware is available";
            Firmware_Send_State(FW_STATE_UPDATEDt, NEW_FWt);
         return;

      }

      if(verzoegerung_fw_update == 1) {return;}

   
      mbedtls_md_type_t fw_checksum_algorithm = mbedtls_md_type_t();
Serial.println(F("++++++++++ Firmware_Shared_Attribute_Received-------------- c"));
...
//Serial.println("------Firmware_Shared_Attribute_Received-------------- 4");
      if (!Firmware_OTA_Subscribe()) {
        return;
      }
//Serial.println("------Firmware_Shared_Attribute_Received-------------- 5");
#if THINGSBOARD_ENABLE_DEBUG
...
#endif // THINGSBOARD_ENABLE_DEBUG
//Serial.println("------Firmware_Shared_Attribute_Received-------------- 6");
      // Calculate the number of chuncks we need to request,
      // in order to download the complete firmware binary
      const uint16_t& chunk_size = m_fw_callback->Get_Chunk_Size();
...
//Serial.println("------Firmware_Shared_Attribute_Received-------------- 7");
