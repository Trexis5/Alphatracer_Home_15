...
//*********************************************************************************************
// Variable, um den letzten gesendeten Fortschritt zu speichern
static int last_sent_progress = -1;

// Gesamtgröße der Firmware in Bytes (m_fw_size sollte vorher definiert sein)
const uint32_t total_firmware_size = m_fw_size;

// Bisher übertragene Bytes berechnen
const uint32_t transferred_bytes = current_chunk * total_bytes;

// Fortschritt in Prozent berechnen
const float progress = (static_cast<float>(transferred_bytes) / total_firmware_size) * 100.0;

// Fortschritt in eine Zeichenkette umwandeln (ohne Kommastelle)
int progress_int = static_cast<int>(progress);
char progress_str[10];
snprintf(progress_str, sizeof(progress_str), "%d%%", progress_int);

// Zustand senden mit Fortschritt nur alle 10% und nur einmal
if (progress_int % 10 == 0 && progress_int != last_sent_progress) {
    char state_message[50];
    snprintf(state_message, sizeof(state_message), "%s %s", FW_STATE_DOWNLOADING, progress_str);
    (void)m_send_fw_state_callback(state_message, nullptr);
